...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (ott && tmdbCandidates.length > 0) {
      ...
      const allowedRatingsStr = allowedRatingsForAge(age).join(' / ');
      prompt = `[영화 추천 요청 — 학교 수업용 학습지 자료]
대상 학생 만 나이: ${age}세 (허용 등급: ${allowedRatingsStr})
선택 OTT: 「${ott.displayName}」 (TMDB로 시청 가능 여부 사전 검증 완료)
관심 주제/테마: 「${topic}」

[★OTT 검증된 후보 목록 — 반드시 이 목록 안에서만 선택★]
아래는 ${ott.displayName}에서 한국에서 실제로 시청 가능한 영화 후보입니다.
이 목록 밖의 영화는 절대 추천하지 마세요.

${candidateList}

[지침]
1. 위 후보가 3편 이상이면 반드시 정확히 3편을 추천하세요. 완벽한 주제 매칭이 어려우면 가장 가까운 영화를 골라 topicConnection에 어떻게 응용할 수 있는지 적어주세요.
2. 후보가 3편 미만일 때만 가능한 만큼 추천.
3. 등급이 [미상]인 영화도 ${age}세에게 부적절해 보이지 않으면 추천 가능 (영화 지식으로 판단).
4. 19세 청소년관람불가가 명백하면 제외.
5. 후보 줄거리를 단서로 「${topic}」와의 연결을 1~2문장으로 적어주세요. 직접 관련이 약하면 교사가 어떻게 수업에 활용할 수 있는지 제안.
6. 반드시 후보 목록의 한국어 제목 표기를 그대로 사용.

반드시 아래 JSON 배열로만 응답하세요(후보가 3편 이상이면 정확히 3개):
[
  {
    "title": "후보 목록의 영화제목",
    "year": "개봉연도",
    "genre": "장르 (예: 애니메이션, 드라마)",
    "koreanRating": "전체관람가" | "12세이상관람가" | "15세이상관람가",
    "reason": "왜 ${age}세에게 좋은지 1~2문장",
    "topicConnection": "「${topic}」와 어떻게 직접 연결되는지 1~2문장",
    "plotSummary": "줄거리 5~7문장 (후보 줄거리를 확장·교육용으로 다듬어주세요)"
  }
]`;
    } else {
